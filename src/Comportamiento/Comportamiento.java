...
public abstract class Comportamiento {
	...
	protected boolean moverDer() {
		...
		Entidad entidadTemp = lab.obtenerEntidad((posicion.obtenerX() / posicion.obtenerAncho()) + 1,
				posicion.obtenerY() / posicion.obtenerAlto());
	...
	protected boolean moverIzq() {
		...
		Posicion posicion = enemigo.obtenerPosicion();
		Laberinto lab = enemigo.obtenerLaberinto();
		Entidad entidadTemp = lab.obtenerEntidad(
				(int) Math.round(Math.ceil((double) posicion.obtenerX() / posicion.obtenerAncho())) - 1,
				posicion.obtenerY() / posicion.obtenerAlto());
		if (posicion.obtenerY() % posicion.obtenerAlto() == 0) {
			if (entidadTemp != null) {
				int velocidadTemp = enemigo.obtenerVelocidad();
	...
	protected boolean moverArriba() {
		...
		Laberinto lab = enemigo.obtenerLaberinto();
		Posicion posicion = enemigo.obtenerPosicion();
		Entidad entidadTemp = lab.obtenerEntidad(posicion.obtenerX() / posicion.obtenerAncho(),
				(int) Math.round(Math.ceil((double) posicion.obtenerY() / posicion.obtenerAlto())) - 1);
	...
	protected boolean moverAbajo() {
		...
		Laberinto lab = enemigo.obtenerLaberinto();
		Entidad entidadTemp = lab.obtenerEntidad(posicion.obtenerX() / posicion.obtenerAncho(),
				(posicion.obtenerY() / posicion.obtenerAlto()) + 1);
		if (posicion.obtenerX() % posicion.obtenerAncho() == 0) {
			if (entidadTemp != null) {
				int velocidadTemp = enemigo.obtenerVelocidad();
				entidadTemp.accept(enemigo.obtenerVisitor());
	...
	private void checkearColision() {
		...
		if (lab.obtenerPersonajePrincipal() != null
				&& enemigo.obtenerPosicion().colisionan(lab.obtenerPersonajePrincipal().obtenerPosicion()))
